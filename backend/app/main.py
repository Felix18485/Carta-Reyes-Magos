...
@app.get("/cartas/{usuario_id}")
def get_cartas_por_usuario(usuario_id: int, db: Session = Depends(get_db)):
    # Filtrar todas las cartas por usuario_id
    cartas = db.query(Carta).filter(Carta.usuario_id == usuario_id).all()
    
    if not cartas:
        raise HTTPException(status_code=404, detail="No se encontraron cartas para este usuario")
    
    response_data = []
    for carta in cartas:
        # Para cada carta, construimos la respuesta con la información deseada
